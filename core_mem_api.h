...
SW_PUBL_API_IMPL inline SwOrZero(SwU8*) swMemFindMem(SwUsz nBytesToScan, const SwU8* memScan, SwUsz nBytesInMemFind, const SwU8* memFind) { SW_DASSERT(memScan && memFind && nBytesInMemFind > 0); for (SwUsz i = 0; (i + nBytesInMemFind) <= nBytesToScan; ++i) { if (swMemEqualMem(nBytesInMemFind, &memScan[i], memFind)) { return (SwU8*)&memScan[i]; } } return swNULL; }
SW_PUBL_API_IMPL inline SwOrZero(SwU8*) swMemFindByte(SwUsz nBytesToScan, const SwU8* memScan, SwU8 byteFind) { SW_DASSERT(memScan); return (SwU8*)__builtin_memchr(memScan, byteFind, nBytesToScan); }
SW_PUBL_API_IMPL inline SwOrZero(SwU8*) swMemFindZero(SwUsz nBytesToScan, const SwU8* memScan) { return (SwU8*)swMemFindByte(nBytesToScan, memScan, 0); }
