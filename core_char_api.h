...
SW_ENSURE(' ' == 0x20 && '!' == 0x21 && '~' == 0x7E, "ensure char literals are ASCII"); // @NOTE: we could make this exhaustive but it's not worth the bloat
...
SW_PUBL_API_IMPL inline SwOrZero(SwU8) swCharToDigitHexOffsetA(SwU8 b) { return swCharInRangeA(b, '0','9') ? '0' : swCharInRangeA(b, 'A','F') ? ('A'-10) : swCharInRangeA(b, 'a','f') ? ('a'-10) : 0; }
...
SW_PUBL_API_IMPL inline SwStr4bA swCharToEscapedStr4bA(SwU8 b) {
    SwStr4bA esc = {0};
    if (swCharInRangeA(b, ' ', '~')) {
        esc.as_bytes_mut[0] = b;
    } else {
        // non printable chars are escaped (printed as hex with leading slash)
        esc.as_bytes_mut[0] = '\\';
        // few exception for octals and common control characters
        switch (b) {
        case 0x00: { esc.as_bytes_mut[1] = '0'; } break; // (zero/nul byte)
        case 0x01: { esc.as_bytes_mut[1] = '1'; } break;
        case 0x02: { esc.as_bytes_mut[1] = '2'; } break;
        case 0x03: { esc.as_bytes_mut[1] = '3'; } break;
        case 0x04: { esc.as_bytes_mut[1] = '4'; } break;
        case 0x05: { esc.as_bytes_mut[1] = '5'; } break;
        case 0x06: { esc.as_bytes_mut[1] = '6'; } break;
        case 0x07: { esc.as_bytes_mut[1] = 'a'; } break; // (alert/audible bell)
        case 0x08: { esc.as_bytes_mut[1] = 'b'; } break; // (backspace)
        case 0x09: { esc.as_bytes_mut[1] = 't'; } break; // (horizontal tab)
        case 0x0A: { esc.as_bytes_mut[1] = 'n'; } break; // (new line / line feed)
        case 0x0B: { esc.as_bytes_mut[1] = 'v'; } break; // (vertical tab)
        case 0x0C: { esc.as_bytes_mut[1] = 'f'; } break; // (form feed)
        case 0x0D: { esc.as_bytes_mut[1] = 'r'; } break; // (carriage return)
        case 0x1B: { esc.as_bytes_mut[1] = 'e'; } break; // (escape)
        default: {
            esc.as_bytes_mut[1] = 'x';
            esc.as_bytes_mut[2] = swToCharDigitHexUpperA(b >> 4);
            esc.as_bytes_mut[3] = swToCharDigitHexUpperA(b & 0xF);
        } break;
        }
    }
    return esc;
